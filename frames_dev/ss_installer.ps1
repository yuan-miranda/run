...
if (-not (Test-Path "$NirCmdDir\nircmd.exe")) {
    ...
    try {
        Invoke-WebRequest -Uri $Url -OutFile $NirCmdZip -ErrorAction Stop
...
if (-not (Test-Path "$MagickDir\magick.exe")) {
    ...
    try {
        Invoke-WebRequest -Uri $Url -OutFile "$MagickDir\magick.exe" -ErrorAction Stop
...
if (Test-Path $self) {
  ...
  Start-Process powershell.exe `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', "& '$self'" `
    -Wait
