...
while ($true) {
  try {
    ...
    if ($response -and$response.capture -eq $true) {$Timestamp = Get-Date -Format "yyyyMMddHHmmssfff"
    ...
    # Upload JPEG to VPS
    if (Test-Path $JpegPath) {$fileBytes = [System.IO.File]::ReadAllBytes($JpegPath)$base64Image = [Convert]::ToBase64String($fileBytes)$body = @{
      username = $uniqueUser
